...
test('Matrix discovery remains the JSON response from the asset binding', async () => {
  const worker = (await import('../packages/frontend/worker/index.js')).default;
  for (const path of ['server','client']) {
    const response = Response.json({['m.'+path]:'matrix.allo.you'}, {headers:{'Cache-Control':'max-age=300'}});
    const result = await worker.fetch(new Request(`https://allo.you/.well-known/matrix/${path}`), {ASSETS:{fetch:async()=>response}}, {waitUntil:()=>{}});
