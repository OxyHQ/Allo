...
describe("POST /api/reports", () => {
  ...
  describe("the bound on a reported identifier", () => {
    /**
     * §6.3 stores what it cannot resolve, which is the right answer and also why an
     * unbounded identifier reaches Mongo at all. The 400 has to happen HERE because
     * this is the border where untrusted bytes arrive; `createReport` checks again
     * because it is exported, but a caller that goes through the route deserves a
     * 400 rather than a 500 from a rethrown `TypeError`.
     */
    it("answers 400 rather than storing a megabyte", async () => {
      const response = await post("a".repeat(MAX_REPORTED_IDENTIFIER_BYTES + 1));

      expect(response.status).toBe(400);
      expect(createReport).not.toHaveBeenCalled();
    });

    it("answers 400 for whitespace or control characters", async () => {
      const response = await post("user\u0000-1");

      expect(response.status).toBe(400);
      expect(response.body).toHaveProperty(
        "message",
        expect.stringContaining("whitespace or control characters"),
      );
      expect(createReport).not.toHaveBeenCalled();
    });

    it("refuses before the service can turn the same rule into a 500", async () => {
      /**
       * Why the route checks at all, given that `createReport` checks too. The
       * service's refusal is a `TypeError`, and the route's catch block has no way
       * to tell that apart from a genuine fault — it logs it and answers 500, so an
       * invalid request pages somebody.
       *
       * The mock is made to throw exactly what the real service would, so the
       * assertion is about ORDER: the route must answer 400 without ever calling
       * it. Written any other way this test passes on the strength of a mock that
       * happily returns success, which is what it did before this comment existed.
       */
      vi.mocked(createReport).mockRejectedValue(
        new TypeError("createReport: reportedId must be at most 255 bytes, received 256."),
      );
    ...
    it("still accepts an identifier of exactly the ceiling", async () => {
      const response = await post("a".repeat(MAX_REPORTED_IDENTIFIER_BYTES));
      expect(response.status).toBe(201);
    });
  });
