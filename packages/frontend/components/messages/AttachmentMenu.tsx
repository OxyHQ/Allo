...
/**
 * An option with no handler is not drawn.
 *
 * Every one of these props is optional, and the honest reading of an absent one
 * is that this build cannot do that thing — location, a contact card and a poll
 * each still need a decision about what Allo would send, not a picker. A tile
 * that opens, closes the sheet and does nothing is the worst of the three
 * possible behaviours: it looks like a bug in the feature rather than the
 * absence of one.
 */
function offered(
  option: Omit<AttachmentOption, 'onPress'>,
...
function isOffered(option: AttachmentOption | undefined): option is AttachmentOption {
