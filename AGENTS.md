...
**The deploy does NOT apply migrations, and merging a schema change does not
ship it.** `deploy-aws.yml` invokes `.github/scripts/deploy-ecs-image.sh` with no
`RUN_MIGRATIONS`, and the script defaults it to `false`
(`deploy-ecs-image.sh:17`), so the migration one-shot never runs. A drizzle
migration therefore reaches production only when somebody applies it
deliberately, and a rollout carrying code that reads a column no migration has
created will fail at runtime rather than at deploy. Check this before assuming a
merged schema change is live.
...
against BOTH stores — a real MongoDB replica set (booted in-process by
`vitest.globalSetup.ts`) and a real Postgres service container — and frontend
typecheck plus `jest`. Running the backend suite locally needs both; a Postgres
alone leaves the moderation suite dead and a Mongo alone leaves the schema suite
dead. `packages/backend/README.md` has the commands.
...
packages/
  ...
  backend/        @allo/backend       Express / drizzle + Mongoose / Socket.io
...
## Data storage: a Mongo→Postgres port, THREE domains done and one left

Both stores are live and the task definition carries both `DATABASE_URL` and
`MONGODB_URI`. Which one a domain uses is not a matter of taste — check here
before writing a query:

| Domain | Store | Where |
|---|---|---|
| social (blocks, restricts, settings, behaviour) | **Postgres** | `db/schema/social.ts` |
| moderation (CrowdSource reports, outbox, enforcement) | **Postgres** | `db/schema/moderation.ts` |
| bridges (Matrix) | **Postgres** | `db/schema/bridges.ts` |
| messaging (conversations, devices, messages) | **Mongo** | `models/`, read by `routes/{conversations,devices,messages}.ts` |

The messaging repositories in `db/messaging/` and the tables in
`db/schema/{conversations,devices,messages}.ts` EXIST and are deliberately
unused — they landed ahead of the switch so the schema could be reviewed on its
own. Do not read their presence as "messaging is on Postgres".

Mongoose survives in exactly five files — the three models, `routes/messages.ts`
and `utils/database.ts` — and that set is the messaging switch's remaining
surface. Note all three messaging ROUTES import the models directly, so a
`grep mongoose` understates the work by two files. Schema decisions live in
`db/schema/CONVENTIONS.md`; the migrator and the two-server test setup are in
`packages/backend/README.md`.

**Before finishing the messaging switch, work through
`db/MONGO-CLAIMS-LEDGER.md`.** It lists every statement in this repo that is true
while Mongo is live and becomes false when it goes — instruction files,
onboarding, the published API docs, and the CI and deploy config no source-level
guard scans. It exists so those are corrected in the change that falsifies them
rather than surviving the migration, which is what happened in oxy-api, Syra and
Homiio.
